    ...
    for (const rawLine of lines) {
        ...
        while (i < trimmed.length) {
            ...
            if (trimmed[i] === '"' || trimmed[i] === "'") {
                ...
                if (i >= trimmed.length)
                    throw new LexError('Unterminated string', lineNum, col);
                i++;
            ...
            if (trimmed[i] >= '0' && trimmed[i] <= '9') {
                ...
                if (trimmed[i] === '%') {
                    tokens.push({ kind: 'PERCENT', value: num, line: lineNum, col });
                    i++;
                }
                else if (trimmed[i] === '°') {
                    tokens.push({ kind: 'DEGREES', value: num, line: lineNum, col });
                    i++;
                }
                else if (rest.toLowerCase().startsWith('cm')) {
                    tokens.push({ kind: 'UNIT_CM', value: num, line: lineNum, col });
                    i += 2;
                }
                else if (rest.toLowerCase().startsWith('ms')) {
                    tokens.push({ kind: 'UNIT_MS', value: num, line: lineNum, col });
                    i += 2;
                }
                else if (trimmed[i]?.toLowerCase() === 's' && !/[a-zA-Z]/.test(trimmed[i + 1] ?? '')) {
                    tokens.push({ kind: 'UNIT_S', value: num, line: lineNum, col });
                    i++;
                }
                else if (trimmed[i]?.toLowerCase() === 'm' && !/[a-zA-Z]/.test(trimmed[i + 1] ?? '')) {
                    tokens.push({ kind: 'UNIT_M', value: num, line: lineNum, col });
                    i++;
                }
                else {
            ...
            if (two === '<=') {
                tokens.push({ kind: 'LTE', value: '<=', line: lineNum, col });
                i += 2;
                continue;
            }
            if (two === '>=') {
                tokens.push({ kind: 'GTE', value: '>=', line: lineNum, col });
                i += 2;
                continue;
            }
            if (two === '==') {
                tokens.push({ kind: 'EQ', value: '==', line: lineNum, col });
                i += 2;
                continue;
            }
            if (two === '!=') {
                tokens.push({ kind: 'NEQ', value: '!=', line: lineNum, col });
                i += 2;
                continue;
            }
            ...
            switch (trimmed[i]) {
                case '<':
                    tokens.push({ kind: 'LT', value: '<', line: lineNum, col });
                    break;
                case '>':
                    tokens.push({ kind: 'GT', value: '>', line: lineNum, col });
                    break;
                case '=':
                    tokens.push({ kind: 'ASSIGN', value: '=', line: lineNum, col });
                    break;
                case '+':
                    tokens.push({ kind: 'PLUS', value: '+', line: lineNum, col });
                    break;
                case '-':
                    tokens.push({ kind: 'MINUS', value: '-', line: lineNum, col });
                    break;
                case '*':
                    tokens.push({ kind: 'STAR', value: '*', line: lineNum, col });
                    break;
                case '/':
                    tokens.push({ kind: 'SLASH', value: '/', line: lineNum, col });
                    break;
                case '(':
                    tokens.push({ kind: 'LPAREN', value: '(', line: lineNum, col });
                    break;
                case ')':
                    tokens.push({ kind: 'RPAREN', value: ')', line: lineNum, col });
                    break;
                default:
