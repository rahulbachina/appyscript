// AppyScript → MicroPython (ESP32) — v3
...
const SENSORS: Record<SensorName, string> = {
...
class ESP32Codegen extends BaseCodegen {
  protected sensorCall(s: SensorName) { return SENSORS[s] ?? `robot.sensor.${s}()` }
  ...
  generate(program: Program, profile: HardwareProfile): GenerateResult {
    ...
    for (const b of defines) {
      ...
      this.indent(); this.emitStatements(b.body); this.dedent()
      this.emitBlank()
    }
    ...
    for (const b of whens) {
      if (b.kind === 'when')
        this.emit(`tasks.append(asyncio.create_task(_handler_${this.trigName(b.trigger)}()))`)
    }
  ...
  private trigToPolling(t: any): { condition?: string; interval?: number } {
    switch (t.kind) {
      ...
      case 'tilted':   return { condition: t.direction ? `robot.sensor.tilted("${t.direction}")` : 'robot.sensor.tilted()' }
      case 'start':    return {}
  ...
  private emitStatement(stmt: Statement) {
    ...
    switch (stmt.kind) {
      case 'move': {
        ...
        this.emit(`robot.move.${stmt.direction}(speed=${sp})`, loc)
        if (ms > 0) { this.emit(`await asyncio.sleep_ms(${ms})`); this.emit('robot.move.stop()') }
        break }
      case 'turn':        this.emit(`robot.move.turn_${stmt.direction}(degrees=${stmt.degrees})`, loc); break
      case 'stop':        this.emit('robot.move.stop()', loc); break
      case 'say':         this.emit(`robot.voice.say(${this.emitStringValue(stmt.text)})`, loc); this.emit('await asyncio.sleep_ms(500)'); break
      case 'play':        this.emit(`robot.voice.play(${JSON.stringify(stmt.sound)})`, loc); break
      case 'show':        this.emit(`robot.display.expression(${JSON.stringify(stmt.expression)})`, loc); break
      case 'show_text':   this.emit(`robot.display.show(${this.emitStringValue(stmt.text)})`, loc); break
      case 'show_number': this.emit(`robot.display.number(${this.emitValue(stmt.value)})`, loc); break
      ...
      case 'save': this.emit(`robot.brain.save(${JSON.stringify(stmt.name)}, ${stmt.name})`, loc); break
      case 'load': this.emit(`${stmt.name} = robot.brain.load(${JSON.stringify(stmt.name)}, 0)`, loc); break
      case 'remember':    this.emit(`robot.brain.remember(${JSON.stringify(stmt.name)}, ${stmt.name})`, loc); break
...
export const esp32Backend = {
  targetId: 'esp32', name: 'ESP32 MicroPython Backend', version: '3.0.0',
  generate(p: Program, prof: HardwareProfile): GenerateResult { return new ESP32Codegen().generate(p, prof) },
}

export function generateESP32(program: Program): string {
  return esp32Backend.generate(program, {
    id:'esp32', name:'ESP32', runtime:'MicroPython', description:'',
