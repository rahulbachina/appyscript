...
import type { Program, Block, Statement, Duration } from '../ast'
...
function bodyHasWait(stmts: Statement[]): boolean {
  for (const stmt of stmts) {
    if (stmt.kind === 'wait') return true
    if (stmt.kind === 'if') {
      if (bodyHasWait(stmt.then)) return true
      if (stmt.else && bodyHasWait(stmt.else)) return true
    }
    if (stmt.kind === 'repeat' && bodyHasWait(stmt.body)) return true
  }
  return false
}

function countConsecutiveKinds(stmts: Statement[], kinds: string[]): number {
  let count = 0
  for (const s of stmts) {
    if (kinds.includes(s.kind)) count++
    else count = 0
  }
  return count
}
...
export class Linter {
  ...
  private usedBehaviours = new Set<string>()
  ...
  lint(program: Program): DiagnosticBag {
    ...
    for (const name of this.definedBehaviours) {
      if (!this.usedBehaviours.has(name)) {
        this.bag.warning(
          CODES.LINT_UNUSED_DEFINE,
          `Behaviour "${name}" is defined but never called with "do ${name}"`,
          undefined,
          `Call it with "do ${name}" or remove it if it's not needed`
        )
      }
    }
  ...
  private collectUsedBehaviours(stmts: Statement[]) {
    for (const stmt of stmts) {
      if (stmt.kind === 'do') this.usedBehaviours.add(stmt.name)
      if (stmt.kind === 'if') {
        this.collectUsedBehaviours(stmt.then)
        if (stmt.else) this.collectUsedBehaviours(stmt.else)
      }
  ...
  private lintBlock(block: Block) {
    if (block.kind === 'when') {
      ...
      if (count === 2) {
        this.bag.warning(
          CODES.LINT_DUPLICATE_HANDLER,
          `Duplicate event handler: "${key}" is handled more than once`,
          block.loc,
          `Only the first matching handler runs on most targets. Merge them into one block.`
        )
      }
    ...
    if (block.kind === 'forever') {
      if (!bodyHasWait(block.body)) {
        this.bag.warning(
          CODES.LINT_INFINITE_NO_WAIT,
          `"forever" loop has no "wait" — it will run at full CPU speed`,
          block.loc,
          `Add "wait 10ms" or "wait 100ms" at the end of the loop to prevent the robot from freezing`,
          { description: 'Add wait 10ms at end of forever block' }
        )
      }
      this.lintStatements(block.body)
    }
  ...
  private lintStatements(stmts: Statement[]) {
    for (let i = 0; i < stmts.length; i++) {
      ...
      if (stmt.kind === 'stop' && next?.kind === 'move') {
        this.bag.warning(
          CODES.LINT_REDUNDANT_STOP,
          `Redundant "stop" immediately before "move" — move already resets motors`,
          stmt.loc,
          `Remove the "stop" line`,
          { description: 'Remove redundant stop' }
        )
      }
      ...
      if (stmt.kind === 'while' && !bodyHasWait(stmt.body)) {
        this.bag.warning(
          CODES.LINT_WHILE_NO_WAIT,
          `"while" loop has no "wait" — it may spin at full CPU speed`,
          stmt.loc,
          `Add "wait 10ms" inside the while loop`
        )
      }
      ...
      if (stmt.kind === 'if') {
        ...
        if (stmt.else) this.lintStatements(stmt.else)
      }
      if (stmt.kind === 'repeat') this.lintStatements(stmt.body)
      if (stmt.kind === 'while') this.lintStatements(stmt.body)
    }
  }

  private triggerKey(trigger: any): string {
    switch (trigger.kind) {
