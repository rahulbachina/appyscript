...
import type { Program, Block, Statement, Condition, Value, SensorName } from '../ast'
import { DiagnosticBag, CODES } from '../diagnostics'
import type { HardwareProfile } from '../plugins'

export class SemanticAnalyser {
  private bag = new DiagnosticBag()
  private definedBehaviours = new Set<string>()
    ...
    for (const block of program.blocks) {
      if (block.kind === 'define') {
        if (this.definedBehaviours.has(block.name)) {
          this.bag.error(
            CODES.SEM_DUPLICATE_DEFINE,
            `Behaviour "${block.name}" is defined more than once`,
            block.loc,
            `Remove or rename one of the "${block.name}" define blocks`
          )
        }
  ...
  private analyseBlock(block: Block, hardware?: HardwareProfile) {
    if (block.kind === 'when') {
  ...
  private analyseTriggerHardware(trigger: any, hardware?: HardwareProfile, loc?: any) {
    if (!hardware) return
    if (trigger.kind === 'sensor') {
      const sensor: SensorName = trigger.sensor
      if (!hardware.sensors[sensor]) {
        this.bag.error(
          CODES.SEM_SENSOR_NOT_AVAILABLE,
          `Sensor "${sensor}" is not available on ${hardware.name}`,
          loc,
          `Remove this handler or choose a target that has a ${sensor} sensor`
        )
      }
    }
  }

  private analyseStatements(stmts: Statement[], hardware?: HardwareProfile) {
    for (const stmt of stmts) {
      this.analyseStatement(stmt, hardware)
    }
  }

  private analyseStatement(stmt: Statement, hardware?: HardwareProfile) {
    const loc = stmt.loc
    switch (stmt.kind) {
      case 'let':
      ...
      case 'set':
        ...
        if (!this.declaredVars.has(stmt.name)) {
          this.bag.warning(
            ...
            `Variable "${stmt.name}" is set but was never declared with "let"`,
            loc,
            `Add "let ${stmt.name} = 0" before this line`,
            { description: `Add let ${stmt.name} = 0 before this block` }
          )
      ...
      case 'remember':
        if (!this.declaredVars.has(stmt.name)) {
          this.bag.warning(
            CODES.LINT_REMEMBER_UNDEFINED,
            `"remember ${stmt.name}" — variable "${stmt.name}" has not been declared`,
            loc,
            `Declare the variable first with "let ${stmt.name} = value"`
          )
        }
        break
      ...
      case 'do':
        if (!this.definedBehaviours.has(stmt.name)) {
          this.bag.error(
            CODES.SEM_UNDEFINED_BEHAVIOUR,
            `Behaviour "${stmt.name}" is not defined`,
            loc,
            `Add a "define ${stmt.name}" block, or check the spelling`,
            { description: `Add: define ${stmt.name}\n  ...\nend` }
          )
        }
        break
      ...
      case 'if':
        ...
        if (stmt.else) this.analyseStatements(stmt.else, hardware)
        break
  ...
  private analyseCondition(cond: Condition, loc: any, hardware?: HardwareProfile) {
    if (cond.kind === 'variable') {
      if (!this.declaredVars.has(cond.name)) {
        this.bag.warning(
          CODES.SEM_UNDEFINED_VARIABLE,
          `Variable "${cond.name}" used in condition but never declared with "let"`,
          loc,
          `Add "let ${cond.name} = 0" before this block`
        )
      }
    }
    if (cond.kind === 'sensor' && hardware && !hardware.sensors[cond.sensor]) {
      this.bag.error(
        CODES.SEM_SENSOR_NOT_AVAILABLE,
        `Sensor "${cond.sensor}" is not available on ${hardware.name}`,
        loc
      )
    }
    if (cond.kind === 'not') this.analyseCondition(cond.condition, loc, hardware)
  ...
  private analyseValue(value: Value, loc: any) {
    if (value.kind === 'variable') {
      if (!this.declaredVars.has(value.name)) {
        this.bag.warning(
          CODES.SEM_UNDEFINED_VARIABLE,
          `Variable "${value.name}" used but never declared with "let"`,
          loc,
          `Add "let ${value.name} = 0" before this block`
        )
      }
    }
